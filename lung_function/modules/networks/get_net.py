    ...
    #     net = build_model_from_cfg(cfg.model)  # pass a config set to this function to build a model
        ...
        if name == "slow_r50":
            ...
            net.blocks[-1].pool.pool = nn.Sequential(
                ...
                nn.Conv3d(432, 432, kernel_size=( 4, 3, 3), stride=(1, 1, 1), bias=False),
                # nn.AdaptiveAvgPool3d(1) 
            )

            # net.blocks[-1].pool.pool = nn.AvgPool3d(kernel_size=(7, 7, 7), stride=1, padding=0)  # change from 16, 7, 7 to 7, 7, 7
            
            net.blocks[-1].pool.post_conv = nn.Conv3d(432, 1024, kernel_size=( 1, 1, 1), stride=(1, 1, 1), bias=False) 
            ...
            net.blocks[-1].proj = nn.Linear(in_features=1024, out_features=nb_cls, bias=True)  # only command previously
