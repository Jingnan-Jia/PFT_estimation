...
from lung_function.modules.networks.models_pcd.pointnet2_utils import square_distance, index_points, query_ball_point
...
def farthest_point_sample_old(xyz, npoint):
    """
    Input:
...
def farthest_point_sample(xyz, npoint):
    ...
    for i in range(npoint):
        centroids[:, i] = farthest
        centroid = torch.tensor(xyz)[batch_indices.to(dtype=torch.long, device=device), farthest.to(dtype=torch.long, device=device), :].view(B, 1, 4)  
        dist = torch.sum((xyz - centroid) ** 2, -1)
        mask = dist < distance
...
class Model(nn.Module):
    ...
    def forward(self, x):  # x.shape # b, d, n	
        # x = x.permute(0, 2, 1) 
        xyz = x.permute(0, 2, 1)
        batch_size, _, _ = x.size()
...
def pointMLP(num_classes=40, points=1024, **kwargs) -> Model:
