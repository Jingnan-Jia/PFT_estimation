...
class ViT3(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels=1,
                 dim_head=256, dropout=0., emb_dropout=0.):
        super().__init__()
        assert image_size % patch_size == 0, 'image dimensions must be divisible by the patch size'
        num_patches = (image_size // patch_size) ** 3  # 1800
        ...
        print('self-pos,patch2-embedding,cls_tokem,dropput', self.pos_embedding[0].shape, self.patch_to_embedding,
              self.cls_token[0].shape, self.dropout)
