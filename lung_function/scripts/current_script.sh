...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --ct_sp='1.5' --net='vgg11_3d' --mode='train' --pretrained_id="SSc-852-853-854-855" --input_mode='vessel' --crop_foreground=True --y_size=240 --x_size=240 --z_size=240 --epochs=500 --workers=4 --test_pat='random_as_ori' --target='FVC-DLCO_SB-FEV1-TLC_He' --remark="pretrained from ssc, vessel as input"
