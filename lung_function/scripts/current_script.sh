...
stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --ct_sp='ori' --net='pointnet_reg' --PNB=7000 --shift_range=0 --batch_size=5 --loss='mse' --mode='train' --dp_fc1_flag='False' --pointnet_fc_ls="1024-512-256" --input_mode='vessel_skeleton_pcd' --position_center_norm='False' --mat_diff_loss_scale=0.1 --epochs=500 --workers=6 --test_pat='random_as_ori' --target='FVC-DLCO_SB-FEV1-TLC_He' --remark="batch size of 5"
