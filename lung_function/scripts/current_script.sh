...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --pretrained_id='0' --input_mode='ct_lower_in_lung' --target='FVC-DLCOc_SB-FEV1-TLC_He' --remark="DLCOc, trained from SSc, background is -1 not 0" &
idx=1; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --pretrained_id='0' --input_mode='ct_upper_in_lung' --target='FVC-DLCOc_SB-FEV1-TLC_He' --remark="DLCOc, trained from SSc, background is -1 not 0"
