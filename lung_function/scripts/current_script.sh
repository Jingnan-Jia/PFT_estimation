...
stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --input_mode='ct_masked_by_vessel_dilated4' --target='FVC-DLCOc_SB-FEV1-TLC_He' --remark="ct_masked_by_vessel_dilated4, DLCOc, trained from SSc, background is -1 not 0"
