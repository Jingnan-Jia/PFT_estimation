...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --ct_sp='1.5' --net='vgg16_3d' --crop_foreground=True --y_size=240 --x_size=240 --z_size=240 --epochs=500 --target='FVC-DLCO_SB-FEV1-TLC_He-Age-Height-Weight-DLCOc_SB-DLCOc_SBc-FEV1/pred-FVC/pred-TLC/pred' --remark="all PFT, vgg16, 500 epochs, 1.5, crop_foreground, pad_ratio = 1.5, no gaussian noise,"
