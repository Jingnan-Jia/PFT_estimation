...
# idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --net='x3d_m' --pretrained_imgnet=True --batch_size=1 --input_mode="ct" --epochs=50 --target='DLCOc_SB-FEV1-FVC-TLC_He' --remark="shuffle=True for training, use conv to replace adaptivepool, by gradually decreasing"
# idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --net='pointnet2_reg' --pretrained_imgnet=False --batch_size=10 --adamw=True --weight_decay=0.0001 --epochs=100 --input_mode='vessel_skeleton_pcd' --lr=0.001 --target='DLCOc_SB-FEV1-FVC-TLC_He' --remark=" shuffle=True for training"
# idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u gcn.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --epochs=100 --target='DLCOc_SB-FEV1-FVC-TLC_He' --remark="shuffle=True for training"
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u combined_run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --net='x3d_m-pointnet2_reg' --batch_size=1 --epochs=100 --input_mode='ct-vessel_skeleton_pcd' --lr=0.001 --target='DLCOc_SB-FEV1-FVC-TLC_He' --remark=" shuffle=True for training"
