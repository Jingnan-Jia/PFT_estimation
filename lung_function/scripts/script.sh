...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --ct_sp='ori' --net='pointnet_reg' --PNB=7000 --batch_size=20 --mode='train' --pointnet_fc_ls="256-128" --dp_fc1_flag='True' --input_mode='vessel_skeleton_pcd' --y_size=240 --x_size=240 --z_size=240 --epochs=500 --workers=6 --test_pat='random_as_ori' --target='FVC-DLCO_SB-FEV1-TLC_He' --remark="7K, batchsize=20,256-128, dp_fc1, remove random scale/shift for input data, loss=mse, batch_size=5, pointnet_reg"
