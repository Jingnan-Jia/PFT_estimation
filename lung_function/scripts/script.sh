...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_${idx}_err.txt 1>${slurm_dir}/slurm-${job_id}_${idx}_out.txt --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --jobid=${job_id} --net='x3d_m' --pretrained_imgnet=False --batch_size=1 --t_stride=2 --input_mode="ct" --target='FEV1-FVC-TLC_He' --remark="3 outputs, from scratch, tstride=1, changed t stride to 2, initialize FC weights, move adaptive layer forward, remove the final pooling"
